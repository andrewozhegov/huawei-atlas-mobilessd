...
*/
bool Mind_people_ssd::CheckNumber(const std::string& value){
...
*/
void Mind_people_ssd::GetSelectImages(const std::string& value){
    //if value is "", no select images
    if(value == ""){
        HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_people_ssd] select images info is null!");
        return;
    }
    std::string token;
    ...
    while (std::getline(tokenStream, token, ','))
    {
        if(CheckNumber(token)){
            ...
            if(frameId >= 0 && frameId < dataset_info_.size()){
                ...
                HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_people_ssd] select image frame id must be in [0, %u), but the input is %u !", dataset_info_.size(), frameId);
            }
        }else{
            HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_people_ssd] select image info must be positive integer, but the input is '%s' !", token.c_str());
        }
    }
    if(select_images_.size() > 0){
        HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_people_ssd] select %u images success!", select_images_.size());
    }
}
...
*/
void Mind_people_ssd::GetRandomImages(){
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_people_ssd] select random %u images success!", data_config_->randomNumber);
}
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_people_ssd::MakeDatasetInfo(){
...
*/
void Mind_people_ssd::GetSelectOrRandomImages(const std::string& selectImages, const std::string& randomNumber){
    //clear vector
    ...
    if(data_config_->runMode == "random"){
        if(CheckNumber(randomNumber)){
            ...
            if(randomNum > 0 && randomNum < dataset_info_.size()){
                ...
                HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_people_ssd] random number must be in (0, %u), but the input is %u !", dataset_info_.size(), randomNum);
            }
        }else{
            HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_people_ssd] random number must be positive integer, but the input is '%s' !", randomNumber.c_str());
        }
    ...
    if(select_images_.size() == 0){
        HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_people_ssd] There is no random or select images, run all images!");
...
*/
void Mind_people_ssd::GetBatchSize(const std::string& value){
    bool check = false;
    ...
    if(!check){
        HIAI_ENGINE_LOG(HIAI_IDE_WARNING, "[Mind_people_ssd] batch size must be positive integer, but the input is '%s'. Use default batch size %u !", value.c_str(), data_config_->batchSize);
    }
}

HIAI_StatusT Mind_people_ssd::Init(const hiai::AIConfig& config, const  std::vector<hiai::AIModelDescription>& model_desc)
{
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_people_ssd] start init!");

    data_config_ = std::make_shared<DataConfig>();
    if(data_config_ == NULL){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] make shared for DataConfig error!");
        return HIAI_ERROR;
    }
    ...
    if(realpath(data_config_->path.c_str(), path) == NULL){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] can not find path!");
        return HIAI_ERROR;
    }
    ...
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_people_ssd] end init!");
    return ret;
}
...
*/
bool Mind_people_ssd::GetImageBuffer(const char* path, uint8_t* imageBufferPtr, uint32_t imageBufferLen, uint32_t frameId){
    bool ret = false;
    FILE * file = fopen64(path, "r");
    if (NULL == file)
    {
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] Error: open file %s failed", path);
        return ret;
    }
    do
    {
        ...
        {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] fseeko64 offset = %u failed in GetImageBuffer", frameId * imageBufferLen);
            break;
        }
        if (imageBufferLen != fread(imageBufferPtr, 1, imageBufferLen, file))
        {
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] fread length = %u failed in GetImageBuffer", imageBufferLen);
            break;
        }
        ret = true;
    } while (0);
...
*/
shared_ptr<EvbImageInfo> Mind_people_ssd::makeEvbImageInfo(int index){
    ...
    if(evbImageInfo == NULL){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] make shared for EvbImageInfo error!");
        return NULL;
    }
    ...
    if(HIAI_OK != get_ret || NULL == imageBufferPtr){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] DMalloc buffer error!");
        return NULL;
    }
    ...
    if(!ret){
        ...
        if(HIAI_OK != get_ret){
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] DFree buffer error!");
        }
        imageBufferPtr = NULL;
        return NULL;
    }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_people_ssd::makeImageInfo(NewImageParaT* imgData, int index) {
    ...
    if(imageBufferPtr == NULL){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] alloc buffer error in makeImageInfo");
        return HIAI_ERROR;
    }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_people_ssd::SendBatch(int batchId, int batchNum, std::shared_ptr<BatchImageParaWithScaleT> imageInfoBatch){
    ...
    do{
        ...
        {
            HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_people_ssd] queue full, sleep 200ms");
            usleep(SEND_DATA_INTERVAL_MS);
        }
    }while(hiai_ret == HIAI_QUEUE_FULL);

    if(HIAI_OK != hiai_ret){
        HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] SendData batch %u failed! error code: %u", batchId, hiai_ret);
    }
    return hiai_ret;
}
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_people_ssd::SendEvbBatch(int batchId, int batchNum, std::vector<std::shared_ptr<EvbImageInfo>>& evbImageInfoBatch){
    HIAI_StatusT hiai_ret = HIAI_OK;
    for(unsigned int i = 0; i < evbImageInfoBatch.size(); ++i){
        ...
        do{
            ...
            {
                HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_people_ssd] hiai graph no useful memory, sleep 200ms");
                usleep(SEND_DATA_INTERVAL_MS);
            }
        }while(HIAI_GRAPH_NO_USEFUL_MEMORY == hiai_ret  || HIAI_GRAPH_INVALID_VALUE == hiai_ret);

        if(HIAI_OK != hiai_ret){
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] SendData image frame id %u failed! error code: %u", evbImageInfoBatch[i]->frame_ID, hiai_ret);
...
*/
int Mind_people_ssd::GetFrameId(int index){
    int frameId = index;
...
* free evb buffer
**/
void Mind_people_ssd::FreeEvbBuffer(const std::vector<std::shared_ptr<EvbImageInfo>>& evbImageInfoBatch, int start){
    HIAI_StatusT ret = HIAI_OK;
    for(unsigned int i = start; i < evbImageInfoBatch.size(); ++i){
        if(evbImageInfoBatch[i]->pucImageData != NULL){
            ...
            if(HIAI_OK != ret){
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] free buffer error!");
            }
...
*/
int Mind_people_ssd::GetTotalCount(){
    int totalCount = dataset_info_.size();
    if(data_config_->runMode != "all"){
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_people_ssd::RunOnSameSide(){
    HIAI_StatusT ret = HIAI_OK;
    ...
    int i = 0;
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_people_ssd] run on %s, run %u images, batch size is %u", data_config_->target.c_str(), totalCount, data_config_->batchSize);
    for(int batchId = 0; batchId < batchNum && i < totalCount; batchId++){
        ...
        if(imageInfoBatch == NULL){
            HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] make shared for BatchImageParaWithScaleT error!");
            return HIAI_ERROR;
        }
        for(int j = 0 ; j < data_config_->batchSize && i < totalCount; j++){
            ...
            if(HIAI_OK != ret) {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "[Mind_people_ssd] Error: make image info frame id %u for batch id %u failed! Stop to send images!",frameId, batchId);
                return ret;
            }
...
* @[return]: HIAI_StatusT
*/
HIAI_StatusT Mind_people_ssd::RunOnDifferentSide(){
    HIAI_StatusT ret = HIAI_OK;
    int totalCount = GetTotalCount();
    int batchNum = totalCount % data_config_->batchSize == 0 ? totalCount / data_config_->batchSize : totalCount / data_config_->batchSize + 1;
    int i = 0;
...
* @[return]: HIAI_StatusT
*/
bool Mind_people_ssd::isOnSameSide(){
...
*/
HIAI_StatusT Mind_people_ssd::SendSentinelImage()
{
    HIAI_StatusT ret = HIAI_OK;
...
*/
HIAI_IMPL_ENGINE_PROCESS("Mind_people_ssd", Mind_people_ssd, INPUT_SIZE)
{
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_people_ssd] start process!");
    ...
    //send sentinel image
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_people_ssd] send sentinel image!");
    ret = SendSentinelImage();
    HIAI_ENGINE_LOG(HIAI_IDE_INFO, "[Mind_people_ssd] end process!");
    return ret;
}
